namespace BrockBot.Commands
{
    ...
    [Command(
        ...
        "",
        "\tExample: `.switch-workers Upland 100`\r\n" +
        "\tExample: `.switch-workers Upland,Ontario 100`",
        "switch-workers"
    )]
    public class SwitchWorkersCommand : ICustomCommand
    {
        private const string SupplyPath = "..\\Accounts\\Accounts - PTC.txt";
        private const string BannedPath = "..\\Accounts\\Banned Level 0s\\";
        private const string AccountsFile = "accounts_{0}.csv";
        ...
        public SwitchWorkersCommand(DiscordClient client, Config config, IEventLogger logger)
        {
            _client = client;
            _config = config;
            _logger = logger;
        ...
        public async Task Execute(DiscordMessage message, Command command)
        {
            ...
            foreach (var cityName in city.Split(','))
            {
                if (SwitchWorkerAccounts(cityName, amount))
                    await message.RespondAsync($"{message.Author.Mention} switched {amount} worker accounts for {cityName}.");
                else
                    await message.RespondAsync($"{message.Author.Mention} failed to switch worker accounts for {cityName}.");
            }
        }
        ...
        private bool SwitchWorkerAccounts(string city, int amount)
        {
            ...
            var newWorkers = goodWorkers.Take(count).ToList();
            goodWorkers.RemoveAll(newWorkers.Contains);
            ...
            File.WriteAllLines(cityWorkersFile, newWorkers);
